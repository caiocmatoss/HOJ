...
const eventDetail = read("src/features/discovery/EventDetailExperience.tsx");
...
assert("Event Detail uses the real event API resource", eventDetail.includes("useEventQuery(eventId)") && read("src/services/api/resources/events.ts").includes("/events/${encodeURIComponent(id)}"));
assert("Event Detail normalizes route id and distinguishes 404", eventDetail.includes("Array.isArray(params.id)") && eventDetail.includes(".trim()") && eventDetail.includes("status === 404") && eventDetail.includes("Evento não encontrado"));
assert("Event Detail has separate initial loading and generic error states", eventDetail.includes("eventQuery.isLoading") && eventDetail.includes("Carregando evento") && eventDetail.includes("Tentar novamente") && eventDetail.includes("eventQuery.refetch"));
assert("Event Detail uses real optional fields only", eventDetail.includes("event.image") && eventDetail.includes("event.description") && eventDetail.includes("event.category") && eventDetail.includes("event.startsAt") && eventDetail.includes("event.endsAt") && eventDetail.includes("event.date") && eventDetail.includes("event.time"));
assert("Event Detail does not promote attendees to RSVP", !eventDetail.includes("participantes") && !eventDetail.includes("Confirmados") && !eventDetail.includes("Pessoas indo"));
assert("Event Detail venue navigation uses internal id", eventDetail.includes("router.push(`/venue/${venue.id}`)") && !eventDetail.includes("/venue/${event.externalId}"));
assert("Event Detail has no event favorite, RSVP or fake ticket CTA", !eventDetail.includes("useFavorite") && !eventDetail.includes("rsvp") && !eventDetail.includes("Comprar ingresso") && !eventDetail.includes("friendsGoing"));
assert("Event Detail does not add realtime or polling", !eventDetail.includes("connectSocket") && !eventDetail.includes("Socket") && !eventDetail.includes("setInterval") && !eventDetail.includes("refetchInterval"));
