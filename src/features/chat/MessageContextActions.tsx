...
type MenuProps = { position: Position; own?: boolean; canForward?: boolean; canEdit?: boolean; myReaction?: ReactionType | null; reactions?: ReactionSummary[]; onReaction?: (type: ReactionType) => void; onReply?: () => void; onForward?: () => void; onEdit: () => void; onDelete: () => void; onCancel: () => void };
export function MessageActionMenu({ position, own = false, canForward = true, canEdit = true, myReaction, onReaction, onReply, onForward, onEdit, onDelete, onCancel }: MenuProps) {
  ...
  return <View ref={menuRef} accessibilityRole="menu" style={[styles.menu, { left: safePosition.x, top: safePosition.y }]}>
    ...
    <Pressable accessibilityLabel="Responder mensagem" accessibilityRole="menuitem" onPress={onReply} style={styles.item}><Text style={styles.label}>Responder</Text></Pressable>
    {canForward ? <Pressable accessibilityLabel="Encaminhar mensagem" accessibilityRole="menuitem" onPress={onForward} style={styles.item}><Text style={styles.label}>Encaminhar</Text></Pressable> : null}
    {own && canEdit ? <Pressable accessibilityLabel="Editar mensagem" accessibilityRole="menuitem" onPress={onEdit} style={styles.item}><Text style={styles.label}>Editar</Text></Pressable> : null}
