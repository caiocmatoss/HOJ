...
export default function FigmaProductionHome() {
  ...
  const mapVenues = query.trim() ? venues : nearbyVenues;
  const mapEvents = query.trim() ? events : nearbyEvents.filter((event) => filter === "now" ? event.isLive : filter === "today" ? new Date(event.date).toDateString() === new Date().toDateString() : true);
  ...
  const items = useMemo<NearbyDiscoveryItem[]>(() => {
    ...
    const sourceVenues = q ? venues : nearbyVenues;
    const sourceEvents = q ? events : nearbyEvents;
