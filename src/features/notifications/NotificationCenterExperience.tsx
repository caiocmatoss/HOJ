...
export default function NotificationCenterExperience({ onClose }: { onClose: () => void }) {
 const user = useUserStore((s) => s.user); const notifications = useNotificationStore((s) => s.notifications); const unreadCount = useNotificationStore((s) => s.unreadCount); const nLoading = useNotificationStore((s) => s.loading); const nError = useNotificationStore((s) => s.error); const loadNotifications = useNotificationStore((s) => s.loadNotifications); const markAsRead = useNotificationStore((s) => s.markAsRead); const inviteQuery = useReceivedInvitesQuery({ page: 1, limit: 100 }); const acceptMutation = useInviteMutation("accept"); const rejectMutation = useInviteMutation("reject"); const invites = inviteQuery.data?.items ?? []; const iLoading = inviteQuery.isLoading; const iError = inviteQuery.error instanceof Error ? inviteQuery.error.message : inviteQuery.error ? "Não foi possível carregar seus convites." : null; const [refreshing, setRefreshing] = useState(false);
 const load = useCallback(async () => { await Promise.allSettled([loadNotifications(), inviteQuery.refetch()]); }, [inviteQuery.refetch, loadNotifications]); useEffect(() => { void load(); }, [load]); const pending = useMemo(() => invites.filter((i) => i.status === "PENDING" && i.receiverId === user?.id), [invites, user?.id]); const error = nError ?? iError; const loading = (nLoading || iLoading) && !notifications.length && !pending.length; const refresh = async () => { setRefreshing(true); await load(); setRefreshing(false); };
