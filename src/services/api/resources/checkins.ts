...
import { checkinKeys, venueKeys } from "@/services/api/query-keys";
...
export function useCheckinMutation() { const client = useQueryClient(); return useMutation({ mutationFn: checkInResource, onSuccess: (result) => { client.setQueryData(checkinKeys.active, result.checkin); void client.invalidateQueries({ queryKey: venueKeys.all }); } }); }
export function useCheckoutMutation() { const client = useQueryClient(); return useMutation({ mutationFn: checkOutResource, onSuccess: () => { client.setQueryData(checkinKeys.active, null); void client.invalidateQueries({ queryKey: ["checkins", "history"] }); void client.invalidateQueries({ queryKey: venueKeys.all }); } }); }
